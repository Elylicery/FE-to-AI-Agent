...
- **数据够多**：训练范式的变化，让模型能够使用的训练数据规模发生了数量级提升。模型从数据本身的结构里自己找学习信号。
- **算力增强**：硬件性能提升、算力增强、分布式训练技术成熟。
- **架构合理**：Transformer 能把"更多参数 + 更多数据 + 更多算力"比较稳定地转化成更好的模型性能。
...
- **Decoder-Only**：给定前面的上下文，继续生成最合适的后续内容。
...
![模型训练过程](images/模型训练过程.png)

- **环节 1：预训练**：让模型具有语言理解与生成能力、基础事实知识、语法、逻辑的能力。
- **环节 2：SFT**：（Supervised Fine-Tuning，监督微调）是在预训练模型之上，使用高质量标注数据进行的有监督微调，让模型学会按照人的要求回答问题。它的本质是：让模型从"会说话"进化到"会按要求说话"。
- **环节 3：RLHF / RLAIF**：RLHF（Reinforcement Learning from Human Feedback）是"基于人类反馈的强化学习"，核心思路是：让人类对多个回答做偏好排序，再让模型朝着"更受偏好"的方向优化。RLAIF（Reinforcement Learning from AI Feedback）则是让 AI 模型来替代一部分人类反馈。这一步的核心目标是：让模型不只是"会答"，而是"更有帮助、更符合偏好、更安全"。
