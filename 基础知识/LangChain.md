...
### 7.3 查询构建
#### 7.2.1 子查询检索器实现动态元数据过滤

根据用户传递的**原始问题**构建相应的**元数据过滤器**呢？这样在搜索的时候带上对应的元数据过滤器，不仅可以压缩检索范围，还能提升搜索的准确性。这个思想其实就**是查询构建**或者称为**自查询**。

查询构建流程：
![[images/Pasted image 20260724135709.png]]

LangChain中的自查询检索器类 selfQueryRetiriever

注意：
自查询检索器 对于**面向特定领域的专用 Agent 效果相对较好**（对通用 Agent 来说效果较差），因为这些领域的文档一般相对来说比较规范，例如：财报、新闻、自媒体文章、教培等行业，**这些行业的数据都能剥离出通用支持过滤与筛选的 元数据/字段，使用自查询检索器能抽象出对应的检索字段信息。**
###  7.4  索引阶段
#### 7.4.1 多向量检索文档

#####  多表征/向量索引

如果能从多个维度记录该文档块的信息，会大大增加该文档块被检索到的概率，**多个维度记录信息 等同于为文档块生成 多个向量**，支持的方法如下：
...
多向量摘要检索的运行流程示例：
![[images/Pasted image 20260724143532.png]]

MultiVector在LangChain中的类实现： Multi  VerctorRetriver类
#### 7.4.2.  父文档检索器实现拆分和存储平衡

**文档拆分与检索的冲突**
- 冲突本质：在RAG应用开发中存在两个相互矛盾的诉求：
    - 需要小型文档（约500token）以保证嵌入向量能准确反映文本含义
    - 又需要足够长的文档（1000-2000token）以保留完整上下文
- 解决方案 **：通过拆分子文档块并检索父文档块的策略实现平衡**
    - 先获取小块数据，再通过元数据中的id查找父文档
    - 适合不可拆分文档或上下文关联性强的场景（如说明书）

 **父文档检索器检索原文档运行流程**
![[images/Pasted image 20260724145246.png]]
langChain中的封装ParentDocumentRetriever

**父文档检索器检索较大块**
上面是使用拆分的文档块检索数据原文档，但是有时候完整文档可能太大，我们不希望按原样检索它们。在这种情况下，我们真正想要做的是先将原始文档拆分成较大的块（例如 1000-2000 个 Token），然后将其拆分为较小块，接下来索引较小块，但是检索时返回较大块（非原文档）。 

运行流程
![[images/Pasted image 20260724150205.png]]

#### 7.4.3  递归文档检索树

##### PAPTOR

使用场景：为LLM的上下文找到一种好的最小化分块方法。在PAPTPOR中，**均衡了多文档、超长上下文、高准确性、超低成本等特性
...
**用树状组织检索的递归抽象处理技术**，采用自下而上方式，通过文本片段聚类和归纳形成分层结构

**运行流程**
![[images/Pasted image 20260724172328.png]]

**主要步骤:**
1. 初始分块：对原始文本进行分块，拆分成合适的大小；
2. 向量化存储：使用文本嵌入模型将文档块转换为高维向量（如1536维），存入向量数据库
3. 降维处理：将高维向量降至2维或3维以降低运算成本
4. 聚类分析：对降维后的向量进行聚类，识别相似文档组；
5. . 摘要生成：合并同类文档文本，使用LLM生成新摘要文档
6. .  递归迭代：重复步骤2-5直到生成符合大小的最终文档

**检索策略**
1. 树遍历策略：2从树的根节点开始，逐层向下进行检索，在每一层它根据查询向量与节点嵌入的余弦相似性选择最相关的前 k 个节点，然后将这些节点的子节点作为下一层的候选节点集，并重复选择过程，直到达到叶节点，最终将所有选中的节点文本进行拼接，形成检索到的上下文。
![[images/Pasted image 20260724172641.png]]
2. 折叠树策略（推荐）：将整个 RAPTOR 树展平为单个层，即所有节点在同一层级上进行比较，计算查询向量与所有节点嵌入的余弦相似性，并选择最相关的前 k 个节点。
![[images/Pasted image 20260724172654.png]]

#### 7.4.4 索引阶段优化策略总结
在 RAG 的 索引阶段，优化策略有分割策略(语义分割、递归字符分割)、多向量/表征检索、RAPTOR递归文档树检索，其中 特定Embeddings 即考虑使用维度更高的文本嵌入模型，并没有复杂的使用技巧。
...
### 7.5 筛选阶段
筛选阶段可采用重排序和纠正性RAG两种策略，其中重排序是使用频率最高、性价比最高的主流优化策略。

####  7.5.1 ReRank重排序

**重排序核心思想**：
对检索到的文档**重新调整顺序**，并且还会增加**剔除无关/多余数据**的步骤
 （RRF算法是重排序中最基础的一种）

**运行流程：**
![[images/Pasted image 20260726094136.png]]

**技术实现：**
在LangChain中重排序是一个DocumentCompressor（压缩组件）
另外LongcontextReorder、Cohere Reranker等高频使用组件有单独封装

 **重排序模型：**
 Cohere、bge-rerank-base、bge-rerank-large、Qwen-rerank等模型
####  7.5.1 纠正性索引增强生成CRAG

纠正性检索增强生成(Corrective Retrival-Augmented Generation，CRAG)是一种先进的自然语言处理技术，旨在提高检索生成方法的鲁棒性和准确性。在CRAG**引入一个轻量级检索评估器评估文档质量，并根据评估结果触发不同的知识检索动作。**

**工作流程:**
![[images/Pasted image 20260726095934.png]]
1.  **检索文档**: 基于用户查询执行检索操作获取相关文档
2. *评估质量**: 使用评估器对每个文档计算量化置信度分数
3. *触发知识检索动作**:
    * 正确: 文档高度相关时进行知识精炼
    * 错误: 文档不相关时启动网络搜索
4. **知识精炼:** 提取关键信息并过滤无关内容
5. **网络搜索**: 补充高质量知识来源
6. ***分解重组**: 将文档解构为关键信息块并重组
7. **生成文本**: 将优化后的知识传递给LLM生成结果

- **循环特性**: 评估不合格时会重新检索或网络搜索，形成反 馈循环机制
- 与传统RAG区别: 增加了质量评估和动态修正环节，能主动纠正检索错误

**实现**
使用LangChraph来构建循环图结构类型的应用  

### 7.6 生成阶段
#### 7.6.1 self-RAG 纠正低质量的检索生成

self-RAG 自我烦死RAG，即对原始查询、检索的内容、生产的内容进行自我反思，根据反思的结果执行不同的操作，例如：**直接输出答案、重新检索、剔除不相关的内容、检测生成内容是否存在幻觉、检测生成内容是否有帮助**等。
self-RAG可以看作是一个拥有自我反思能力的智能体，这个智能体主要用来依据相关知识库回复用户问题，自我迭代，知道输出满意的结果

self-RAG步骤：
![[images/Pasted image 20260726103550.png]]
1. **按需检索(Retrieval as Needed)：**  查询某些内容时输出检索query，查询某些内容时直接给答案
2. **并行生成(Parallel Generation)：**
    - 同时使用prompt和检索内容生成输出
    - 并在过程中触发多种反思：文档相关性反思、内容幻觉检测
3. **评估选择：** 对生成内容进行质量评估，选择最佳文档段落输出
### 7.7 RAG优化总结
